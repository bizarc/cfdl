...
const benchmarkTitles = {
  ...
  "credit/fnma_remic_2019_2_g3": "Credit: Fannie Mae REMIC with a stripped coupon, at six prepayment speeds",
...
// --- Stage the machine docs (docs/32 Phase 2) -------------------------------
// Generated by tools/gen-machine-docs.py (make machine-docs), committed under
// docs/machine/, and served verbatim: llms.txt and llms-full.txt at the site
// root per the llms.txt convention, the bundle and repair catalog under
// /machine/. Same staleness contract as the schemas above.
const stagedMachineDocs = {
...
for (const [sourceName, sitePath] of Object.entries(stagedMachineDocs)) {
  ...
  const content = fs.readFileSync(src, "utf8");
  const target = path.resolve(siteDir, sitePath);
  if (checkMode) {
    if (!fs.existsSync(target) || fs.readFileSync(target, "utf8") !== content) {
      throw new Error(`Staged machine doc is stale or missing: site/${sitePath}`);
    }
  } else {
