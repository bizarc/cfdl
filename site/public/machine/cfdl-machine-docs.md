...
# CFDL machine documentation bundle — v0.9.0
...
row_clause      = "category" STRING
                ...
                | "display" IDENT
                | "depth" NUMBER ;
slice_clause    = "entity" entity_ref
                | "type" ( QNAME | IDENT )
                | "line" IDENT
                | "category" STRING
                | "stream" STRING
...
(* --- assumptions --- *)
assume_stmt     = "assume" IDENT [ ":" IDENT ]
                  ( "=" expr | "~" dist_expr ) [ "within" list_number ] ;
...
contract_stmt   = "contract" dotted_name [ qname ] [ "on" "entity" entity_ref ]
                  contract_block ;
...
(* --- standalone streams --- *)
(* `direction` and `currency` are optional: a stream that states neither is an
   inflow in the model's reporting currency, which is what most core-spelled
   streams are. *)
stream_stmt     = "stream" dotted_name
...
(* What this flow IS: the one thing that says how aggregation folds it
   (docs/35). A stream declares its own; a pack rule assigns one to each
   stream it lowers. *)
stream_category = "category" qname ;
...
waterfall_block = "{" ( schedule_stmt "from" expr | "from" expr schedule_stmt )
                  { waterfall_step } "}" ;
...
(* A step pays a party or a declared account, and may name the contract
   claim it settles — which is what lets a class be paid its own interest
   rather than a number that happens to equal it. *)
waterfall_step  = "pay" IDENT "to" ( "account" IDENT | entity_ref )
                  [ "for" "contract" qname "line" IDENT ]
                  "=" expr ;
...
schedule_on     = "on" date_lit [ placement ] [ "net" INT [ "days" | "months" ] ]
                  [ schedule_opts ] ;
...
(* At least one of the two: a schedule supplies the occasions, `when`
   filters them, and an event with neither has no occurrence to fire on. *)
event_stmt      = "event" qname
                  ( "schedule" schedule_expr [ "when" expr ]
                  | "when" expr )
                  event_block ;
...
(* --- options --- *)
(* An option is a contract with an election (docs/01 §14, docs/40 §4.8). It is
   written ON something — an entity, or the agreement the right is over: a
   renewal on a lease, a prepayment on a loan — and is BETWEEN parties in the
   roles its election type declares. Its `terms` are checked against that
   type's effective fields as a contract's are, and the election and the
   payoff read them as `contract.<term>`; written on a contract, the
   agreement's terms are readable the same way where the option states none
   of its own. `exercisable N times` is how often the right may be exercised
   (once by default); a `schedule` supplies the occasions the election is
   tested at (a Bermudan right), and without one an occasion is the
   election's rising edge while the option is held. The action statements
   are what an exercise DOES beyond paying — the event's vocabulary, run on
   each exercise. The items may appear in any order. *)
option_stmt     = "option" qname
                  [ "on" ( "entity" entity_ref | "contract" qname ) ]
                  "type" qname
                  [ "exercisable" [ INT "times" ] [ "in" qname ] ]
                  option_block ;

option_block    = "{" option_item* "}" ;
option_item     = parties_block
                | terms_block
                | "schedule" schedule_expr
                | "exercise" "when" expr
                | "payoff" expr
...
money_lit       = number ( IDENT | STRING ) ;
                  (* e.g. `42000 USD`; quoted where the unit is not an
                     identifier, as in `250000 "MWh/yr"` *)

list            = "[" [ literal { "," literal } ] "]" ;

(* NOT IMPLEMENTED. The parser accepts an inline map literal in no form,
   with or without `=` between a key and its value. Found by
   `check-grammar-generation.py`, which is the only thing that ever
   asked. Kept rather than deleted because `literal` still names it and
   the shape is the one a map literal would take; both grammar gates
   respect the annotation and stop promising it. *)
map_inline      = "{" [ qname "=" literal_or_expr
                        { [ "," ] qname "=" literal_or_expr } ] "}" ;

list_number     = "[" [ "-" ] number "," [ "-" ] number "]" ;
                  (* a low and a high; never one, never three *)

entity_ref      = IDENT "." IDENT { "." IDENT } ;
qname           = IDENT { "." IDENT } ;
(* A DECLARED name, as distinct from a reference: at least two segments.
   A stream, a contract and a waterfall are namespaced by what declares
   them, and the parser refuses a bare identifier for each. *)
...
(the engine substitutes 0 / false and records the warning), except in a field's
rule, where a failure is fatal (`E5032`). An unknown name is `EXPR_UNKNOWN_NAME`
and fails the run (`E5031`). Both codes are registered in `docs/08` §7.7.
    ...
    "ResultsGraph": {
      ...
      "properties": {
        ...
        "contracts": {
          ...
          "description": "The contracts the model declared, each resolved to its type and master (docs/40 §8). Absent when the model declares none."
        }
      }
    },
    "SliceSelection": {
...
- **`set` may name the ACCOUNT a master declares** (`docs/42` §3.5):
  `set balance = 0` on the `credit.loan` machine. The entity carries
  `<entity>.balance` because its contract opened it, so the write is a
  movement of the opening balance to that value — a write-off on repurchase
  — journaled as a `move` from the machine, and every row reading
  `prev.balance` that period reads the new opening. A FIELD ROLE
  (`field_role = "balance"` beside `field_name`) is the narrower companion:
  a private field the machine ends together with the account — the lagged
  twin a loan's recoveries read, so a repurchased loan recovers nothing for
  its seller. The compiler resolves the name per entity to the account and
  to every field that plays the role; a name nothing on the transitioning
  entity carries is refused (`E1359`) for a model's action and is a no-op
  for a pack's.
...
- `E1376_UNKNOWN_REFERENCE` — a reference names something this model does not declare: a term of type `contract` or `account` (a guarantee's `covered`, a note's `principal_account`), a waterfall step's `for contract`, or an option's `on contract`. Refused with the near miss rather than read as zero; a reference is what the compiler can check (`docs/40` §3).
...
# Added 2026-09-16. The list above is what the audit measured; these are forms it
# never met, confirmed to pass the gate before being added.
colour = "color"
colours = "colors"
coloured = "colored"
analyse = "analyze"
...
term = "slice"
category = "language"
definition = "A named, deliberately partial selection of a model's streams, with figures computed over the selection."
note = "A slice carries no reconciliation block: partial by design, and seen to be. Contrast statement, whose defining property is completeness."
...
[[not_approved]]
# This entry BUILDS rule W6. The terms below are the gate, not a copy of it:
# tools/check-site-voice.py compiles them at run time, so adding one here is the
# whole change. Until 2026-09-16 the list lived twice, hardcoded in the tool and
# restated here under a comment claiming the two could not drift, while nothing
# read this copy at all.
id = "W6"
why = "reads as marketing rather than documentation"
mode = "gate"
terms = [
  "blazing", "blazingly", "lightning-fast", "lightning fast", "world-class",
  "world class", "cutting-edge", "cutting edge", "best in class",
  "state-of-the-art", "state of the art", "revolutionary", "seamless",
  "seamlessly", "effortless", "effortlessly", "game-changer", "game changer",
  "game-changing", "game changing", "best-in-class",
  "unparalleled", "robust and", "powerful and", "simply put", "crown jewel",
  # Added 2026-09-16. Intensifiers and self-praise the original list missed.
  "incredibly", "extremely", "remarkably", "dramatically", "vastly", "hugely",
  "massively", "elegantly", "beautifully", "trivially", "effortless",
  "just works", "magical", "magically", "turnkey", "bulletproof", "rock-solid",
  "battle-tested", "enterprise-grade", "supercharge", "supercharged",
]
...
# Contextual, like `mm`: banned as a valuation multiple (a digit, then the sign),
# correct in spaced arithmetic and in grid dimensions. X1 carries that context.
context = "multiple"
...
# ---------------------------------------------------------------------------
# Rules carried as data.
#
# Each entry builds one rule in tools/check-site-voice.py. `mode` decides what a
# finding costs:
#
#   gate    — fails the build. For rules that are mechanical and measure zero,
#             where a hit is a defect and never a judgment call.
#   review  — reported, never fails. For rules that are worth seeing and cannot
#             be made precise enough to block on. The gate's founding rule is
#             that a gate flagging judgment gets disabled, so judgment is
#             surfaced here instead of being either enforced or dropped.
#
# A review rule is promoted to a gate once its findings have been worked through
# and its list calibrated. That is the intended direction of travel.
# ---------------------------------------------------------------------------

[[rule]]
id = "N5"
mode = "gate"
why = "names a competitor product"
terms = ["Argus", "Intex", "PySAM", "System Advisor Model", "NREL's SAM"]
reason = """
Comparison against these is internal work. The parity documents discuss them at
length and none of that reaches a reader. Naming one on a published page turns a
measurement into a competitive claim, which is not what the documentation is for.
"""
use_instead = "Say what CFDL does. Let the reader make the comparison."
waivable = false
occurrences = 0

# Deliberately NOT in the list above: Excel. It is named across the published
# pages as an interchange format and as a technique — the rounding it produces,
# the compatibility mode that reproduces it, the circularity its users recognise.
# Those are facts a reader needs. A claim of superiority over it is caught by N8.

[[rule]]
id = "N8"
mode = "review"
why = "claims superiority rather than stating what the software does"
terms = [
  "better than", "faster than", "simpler than", "cleaner than", "safer than",
  "more accurate than", "more powerful than", "superior to", "outperforms",
  "outperform", "beats", "no other", "the only tool", "the only language",
  "unlike most", "unlike other", "replaces the need for", "industry-leading",
]
reason = "A comparison the reader did not ask for, and usually cannot check."
use_instead = "State the behaviour and let it stand on its own."

[[rule]]
id = "N9"
mode = "review"
why = "discloses that the product is unfinished"
terms = [
  "not yet implemented", "not yet supported", "not yet built", "not yet shipped",
  "cannot yet", "does not yet", "no support for", "currently lacks",
  "for now", "in a future release", "future work", "planned for",
  "a known limitation", "this limitation", "work around this",
]
reason = """
docs/16 §2.2 settles this: the repository records why, the site publishes the
conclusions. A sentence about what the software cannot do yet tells a reader the
tool is a work in progress, which is a statement about the project rather than
about the product.

The word `cannot` alone is deliberately absent. A specification legitimately says
what is not permitted, and the temporal markers are what distinguish a rule from
an apology.
"""
use_instead = "State what the software does. Leave the roadmap in the repository."

[[rule]]
id = "N7"
mode = "review"
why = "gives software a mind"
subjects = [
  "model", "engine", "language", "compiler", "parser", "pack", "gate", "checker",
  "stream", "contract", "waterfall", "schedule", "account", "scenario", "run",
]
verbs = [
  "wants", "want", "knows", "know", "thinks", "think", "believes", "believe",
  "hopes", "hope", "cares", "care", "likes", "like", "prefers", "prefer",
  "tries", "try", "struggles", "struggle", "remembers", "remember", "forgets",
  "forget", "understands", "understand", "learns", "learn", "feels", "feel",
  "decides", "decide", "expects", "expect", "worries", "worry",
  "happily", "patiently", "eagerly", "politely", "cheerfully",
]
reason = """
A model that wants something is a metaphor standing in for the mechanism, and the
mechanism is what the reader came for.

`refuses` and `rejects` are deliberately absent. A compiler that refuses a program
is the precise word for what it does, not a personification.
"""
use_instead = "Name what the software does: accepts, rejects, evaluates, reports."
```
