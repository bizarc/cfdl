...
CFDL 0.9.0. 19 of the registered benchmark
...
// ----------------------------------------------------------------- the land
// FACT. Deed 4491/2257, 2011-09-30, one price across the fee parcels.
assume land_price = 67000000.0
...
// construction window.
assume hard_buildings    = 301693050.0
assume hard_garage       = 33184000.0
assume fire_station      = 7454800.0
assume park_and_street   = 14000000.0
assume soft_costs        = 60576414.5
assume contingency       = 17816592.5
assume developer_fee     = 17994713.1
...
assume utility_fund         = 143303.0
assume tdm_buy_in           = 14000.0
assume public_art           = 250000.0
assume green_building_fund  = 2500.0
assume affordable_housing   = 12000000.0
...
assume construction_start  = months_between(parse_date("2011-09"), parse_date("2018-04"))
assume construction_months = 39.0
assume delivery_aubrey     = months_between(parse_date("2011-09"), parse_date("2021-07"))
assume delivery_evo        = months_between(parse_date("2011-09"), parse_date("2021-09"))
assume sale_month          = months_between(parse_date("2011-09"), parse_date("2022-05"))
assume condo_first_close   = months_between(parse_date("2011-09"), parse_date("2021-08"))
assume condo_last_close    = months_between(parse_date("2011-09"), parse_date("2024-06"))
...
assume loan_commitment      = 380000000.0
assume loan_rate            = 0.065
assume equity_share_of_cost = 0.35
...
assume aubrey_units       = 331.0
assume evo_units          = 455.0
assume aubrey_rent        = 3522.0      // DERIVED, $/unit/month
assume evo_rent           = 3273.0      // DERIVED
assume aubrey_assessed    = 229060900.0 // FACT, 1/1/2022
assume evo_assessed       = 290055300.0 // FACT
assume aubrey_retail_sf   = 9788.0      // FACT
assume evo_retail_sf      = 13891.0     // FACT
assume other_income_pct   = 0.04
assume vacancy            = 0.08        // FACT
assume expenses_per_unit  = 7511.0      // FACT, $/unit/year
assume parking_per_space  = 150.0       // FACT, $/space/month
assume parking_ratio      = 0.8        // FACT, spaces per unit
assume retail_rent_psf    = 48.0        // $/sf/year
assume retail_occupancy   = 0.9
assume effective_tax_rate = 0.0113      // FACT, on assessed value
assume units_per_month    = 25.0        // lease-up pace, each tower from its delivery

// ---------------------------------------------------------------- the exit
// FACT: both towers sold 2022-05-17 as ONE transaction; only the deed
...
// ASSUMPTIONS.
assume aubrey_price       = 266455000.0
assume evo_price          = 334642240.0
...
// closings' own months.
curve pierce_sellout to 2024-12 {
  2021-08: 2056400.00
  2021-09: 8109000.00
  2021-10: 6243000.00
  2021-11: 16820000.00
...
// ---- aubrey: delivered 2021-07, sold 2022-05 — still in lease-up. Each
...
contract credit.loan.bridge_f on entity asset.buyer {
...
contract credit.loan.a on entity asset.loan_a {
...
contract credit.loan.b on entity asset.loan_b {
  term 2026-01..2056-12
  terms {
    principal = 30000000
    interest_rate = 0.08
...
contract credit.loan.c on entity asset.loan_c {
  term 2026-01..2056-12
  terms {
    principal = 20000000
    interest_rate = 0.08
...
contract credit.loan.d on entity asset.loan_d {
  term 2026-01..2056-12
  terms {
    principal = 10000000
    interest_rate = 0.08
...
stream credit.trust.servicing_fee on entity container.trust outflow currency USD {
  ...
  amount = 0.01 / 12.0 * (inputs.initial_pool
             - if(time.t == 0.0, 0.0,
                  series_sum("credit.loan.sched_principal.*", 0, time.t - 1)
                  + series_sum("credit.loan.prepay.*", 0, time.t - 1)))
}
