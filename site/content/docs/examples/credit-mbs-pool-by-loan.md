...
//
// The four balances are deliberately uneven. Four equal loans would agree with
// the pool under any aggregation that happened to divide by four.
//
// The loans state a balance and a coupon but not a term, because `term` is a
// keyword and an attribute cannot be called one — `Credit.Asset.Loan` declares
// the field and no model can set it. The term the schedule uses is the
// contract's `term_months`, so nothing here depends on it.
