...
//
// THE AGREEMENT IS THREE-WAY. The laboratory publishes this model as a
// spreadsheet, and an independent contributor has ported it to Python. Both
// were run. Three-way agreement rules out something a two-way agreement leaves
// open: that CFDL and one reference share a mistake.
//
// THE TARIFF IS AN INPUT HERE. The reference solves for the tariff that clears
// a target equity return, sweeping the rate until net present value crosses
// zero. That solved rate — 23.15 c/kWh — is carried across as a stated constant,
// and what this case asserts is the cash flow at that rate, period by period.
//
...
//
// A percentage of revenue rather than a stated amount, so it is written from
// its own drivers: the same production and price the tariff earns, times 3%.
//
// The category is `operating.expense.om`, which is where the energy pack folds
// operating cost. `domain.energy.opex` globs `operating.expense.*`, so the
// royalty reaches opex and NOI exactly as the four expense contracts do.
// ---------------------------------------------------------------------------
