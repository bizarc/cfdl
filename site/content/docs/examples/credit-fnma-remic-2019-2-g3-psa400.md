...
# Credit: Fannie Mae REMIC at 400% PSA

Group 3 of a Fannie Mae REMIC at 400% PSA — a fast pool, and the column whose weighted average life pins the timing convention hardest.
...
A fast pool at 24% CPR. This is the column that pins the timing convention hardest: its published weighted average life of 2.9 sits far enough from the model's month-end axis that the ~0.015-year axis difference between the engine's measure and the deal's 25th-of-month distributions is visible, where at the other speeds it hides inside the print rounding.
...
to 400% PSA and asserts the decrement column the supplement publishes for it.
...
400% PSA column. See the base case's `SOURCE.md`.

## What it exercises

The same model as the base case with one term changed — `psa_speed` at 400% of the standard curve. What the seven
...
**170 asserted values**, every one within the half-percent floor the
table's whole-percent rounding sets. Worst balance disagreement
**0.475 percentage points** against the 0.5 floor.

| | |
|---|---|
| Class AB weighted average life | **2.9608**, published **2.9** |
...
**2 years published only as '*'.** The table marks a balance above zero and below half a percent with a star rather than a value, so those cells assert nothing — there is no published number to hold the model to, and inventing one would assert the model against itself.
...
version 0.1
model "fnma-remic-2019-2-g3-psa400"
use pack "credit" version "0.1.0"
time calendar monthly from 2019-02 for 361

// GROUP 3 OF FANNIE MAE REMIC TRUST 2019-2 AT 400% PSA, against the same
...
entity asset pool : Credit.Asset.Loan {
  ...
  balance init 148372434.0
               ...
               * (1.0 - cpr_to_periodic(min(4.0 * 0.002 * max(1.0, min(176.0, 30.0)), 1.0), 12.0))
          next if(time.t < 173.0,
                  prev * (1.0 - ((-pmt(0.0045425, 173.0 - time.t, 1.0)) - 0.0045425))
                       * (1.0 - cpr_to_periodic(min(4.0 * 0.002 * max(1.0, min(time.t + 176.0, 30.0)), 1.0), 12.0)),
                  0.0)
}
...
Checked period by period: **6 series** across **58 periods** — **170 values** in all, each within the tolerance shown.
...
| `domain.credit.wal_years` | 2.9 | ±0.07 |
| `model.total` | 170,337,628.95 | ±1 |
| `metric.ab_wal` | 2.9 | ±0.07 |
