...
# Credit: Fannie Mae REMIC at 1000% PSA

Group 3 of a Fannie Mae REMIC at 1000% PSA — the table's fastest column, past half the pool prepaying every year.
...
The table's fastest column: 60% CPR, more than half the pool prepaying each year, the class effectively gone in three. Nine of the table's years print '*' rather than a value, the most of any column.
...
to 1000% PSA and asserts the decrement column the supplement publishes for it.
...
1000% PSA column. See the base case's `SOURCE.md`.

## What it exercises

The same model as the base case with one term changed — `psa_speed` at 1000% of the standard curve. What the seven
...
**135 asserted values**, every one within the half-percent floor the
table's whole-percent rounding sets. Worst balance disagreement
**0.455 percentage points** against the 0.5 floor.

| | |
|---|---|
| Class AB weighted average life | **1.0609**, published **1.1** |
...
**9 years published only as '*'.** The table marks a balance above zero and below half a percent with a star rather than a value, so those cells assert nothing — there is no published number to hold the model to, and inventing one would assert the model against itself.
...
version 0.1
model "fnma-remic-2019-2-g3-psa1000"
use pack "credit" version "0.1.0"
time calendar monthly from 2019-02 for 361

// GROUP 3 OF FANNIE MAE REMIC TRUST 2019-2 AT 1000% PSA, against the same
...
entity asset pool : Credit.Asset.Loan {
  ...
  balance init 148372434.0
               ...
               * (1.0 - cpr_to_periodic(min(10.0 * 0.002 * max(1.0, min(176.0, 30.0)), 1.0), 12.0))
          next if(time.t < 173.0,
                  prev * (1.0 - ((-pmt(0.0045425, 173.0 - time.t, 1.0)) - 0.0045425))
                       * (1.0 - cpr_to_periodic(min(10.0 * 0.002 * max(1.0, min(time.t + 176.0, 30.0)), 1.0), 12.0)),
                  0.0)
}
...
Checked period by period: **6 series** across **51 periods** — **135 values** in all, each within the tolerance shown.
