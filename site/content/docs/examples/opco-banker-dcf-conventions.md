...
a sell-side banker's DCF of an enterprise-software business. Reconciled against the analysis disclosed in a public merger filing — free, citable, and unusually complete: the filing gives the unlevered free cash flow build-up line by line, the discount rate, the terminal multiple, the discounting convention, the dilution assumption, AND the answer grid. See NOTES.md for what it is and what reproducing it found. There is deliberately no reference_gen.py. Every figure asserted here is the filing's; a second implementation of our own is what an external source replaces. The asserted metric is the centre of the disclosed 3x3 grid — 10.375% WACC, 25.0x terminal multiple, published implied enterprise value $15,764mm. That one cell exercises every convention in the case: the mid-period stub, the four full-year placements, and a terminal value discounted whole rather than mid-period. All nine cells were reconciled (worst +1.17 on $19bn, which is the filing's own whole-$mm rounding); the other eight need a different discount rate or multiple per run, so they live in NOTES.md. tolerance 1.0 — a million on sixteen billion, set by the filing's rounding rather than by the engine. The engine's own agreement is +0.16. Units are $mm throughout, matching the filing.
...
stream opco.ufcf.stub_fy25 on entity legal.target inflow currency USD {
  schedule every month mid from 2025-02 to 2025-02
  amount = 367
}

stream opco.ufcf.fy26 on entity legal.target inflow currency USD {
  schedule every month from 2025-12 to 2025-12
  amount = 481 * (1 - inputs.dilution_fy26)
}

stream opco.ufcf.fy27 on entity legal.target inflow currency USD {
  schedule every month from 2026-12 to 2026-12
  amount = 502 * (1 - inputs.dilution_fy27)
}

stream opco.ufcf.fy28 on entity legal.target inflow currency USD {
  schedule every month from 2027-12 to 2027-12
  amount = 639 * (1 - inputs.dilution_fy28)
}

stream opco.ufcf.fy29 on entity legal.target inflow currency USD {
  schedule every month from 2028-12 to 2028-12
  amount = 736 * (1 - inputs.dilution_fy29)
}

stream opco.exit.value on entity legal.target inflow currency USD {
