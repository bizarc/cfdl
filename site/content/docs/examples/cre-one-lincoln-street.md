...
// program keeps running into, and it is why this case takes Exhibit 7.
//
...
// The case has always RUN with `--pack cre` for its domain metrics; it now has
// to DECLARE the pack too. Metrics are a post-pass over results and needed the
// pack only at run time, but per-period subtotals are lowered into the IR, so
// they need it at compile time. A model that wants a pack's reporting has to
// say it uses the pack.
