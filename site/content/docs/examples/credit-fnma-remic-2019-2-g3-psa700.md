...
# Credit: Fannie Mae REMIC at 700% PSA

Group 3 of a Fannie Mae REMIC at 700% PSA — a refinancing wave, with the class under two years of average life.
...
A refinancing wave — 42% CPR, the class under two years of average life, and six of the table's years published only as '*', an outstanding balance above zero and below half a percent that the case leaves unasserted.
...
to 700% PSA and asserts the decrement column the supplement publishes for it.
...
700% PSA column. See the base case's `SOURCE.md`.

## What it exercises

The same model as the base case with one term changed — `psa_speed` at 700% of the standard curve. What the seven
...
**150 asserted values**, every one within the half-percent floor the
table's whole-percent rounding sets. Worst balance disagreement
**0.427 percentage points** against the 0.5 floor.

| | |
|---|---|
| Class AB weighted average life | **1.6920**, published **1.7** |
...
**6 years published only as '*'.** The table marks a balance above zero and below half a percent with a star rather than a value, so those cells assert nothing — there is no published number to hold the model to, and inventing one would assert the model against itself.
...
version 0.1
model "fnma-remic-2019-2-g3-psa700"
use pack "credit" version "0.1.0"
time calendar monthly from 2019-02 for 361

// GROUP 3 OF FANNIE MAE REMIC TRUST 2019-2 AT 700% PSA, against the same
...
entity asset pool : Credit.Asset.Loan {
  ...
  balance init 148372434.0
               ...
               * (1.0 - cpr_to_periodic(min(7.0 * 0.002 * max(1.0, min(176.0, 30.0)), 1.0), 12.0))
          next if(time.t < 173.0,
                  prev * (1.0 - ((-pmt(0.0045425, 173.0 - time.t, 1.0)) - 0.0045425))
                       * (1.0 - cpr_to_periodic(min(7.0 * 0.002 * max(1.0, min(time.t + 176.0, 30.0)), 1.0), 12.0)),
                  0.0)
}
...
Checked period by period: **6 series** across **54 periods** — **150 values** in all, each within the tolerance shown.
...
| `domain.credit.wal_years` | 1.7 | ±0.07 |
| `model.total` | 161,035,147.51 | ±1 |
| `metric.ab_wal` | 1.7 | ±0.05 |
