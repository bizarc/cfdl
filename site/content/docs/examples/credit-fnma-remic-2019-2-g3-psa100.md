---
id: benchmark-credit-fnma-remic-2019-2-g3-psa100
title: "Credit: Fannie Mae REMIC at 100% PSA"
slug: "/docs/examples/credit-fnma-remic-2019-2-g3-psa100"
description: "Group 3 of a Fannie Mae REMIC at 100% of the standard prepayment curve — the slow column of the issuer's decrement table."
source: benchmarks/credit/fnma_remic_2019_2_g3_psa100
---

# Credit: Fannie Mae REMIC at 100% PSA

Group 3 of a Fannie Mae REMIC at 100% of the standard prepayment curve — the slow column of the issuer's decrement table.
...
The standard prepayment curve at exactly its own pace — the slow bookend of the six columns that share the Pricing Assumptions. The pool is seasoned past the ramp, so 100% PSA is a flat 6% CPR, and the class retires by January 2034 having spent six years paying down.
...
to 100% PSA and asserts the decrement column the supplement publishes for it.
...
100% PSA column. See the base case's `SOURCE.md`.

## What it exercises

The same model as the base case with one term changed — `psa_speed` at 100% of the standard curve. What the seven
...
**0.414 percentage points** against the 0.5 floor.

| | |
|---|---|
| Weighted average life | **6.1125**, published **6.1** |
...
version 0.1
model "fnma-remic-2019-2-g3-psa100"
use pack "credit" version "0.1.0"
time calendar monthly from 2019-02 for 361

// GROUP 3 OF FANNIE MAE REMIC TRUST 2019-2 AT 100% PSA, against the same
...
entity asset pool : Credit.Asset.LoanPool {
  ...
  balance init 148372434.0
               ...
               * (1.0 - cpr_to_periodic(min(1.0 * 0.002 * max(1.0, min(176.0, 30.0)), 1.0), 12.0))
          next if(time.t < 173.0,
                  prev * (1.0 - ((-pmt(0.0045425, 173.0 - time.t, 1.0)) - 0.0045425))
                       * (1.0 - cpr_to_periodic(min(1.0 * 0.002 * max(1.0, min(time.t + 176.0, 30.0)), 1.0), 12.0)),
                  0.0)
}
