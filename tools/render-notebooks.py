...
def render(path: pathlib.Path, notebook: dict) -> tuple[str, dict[str, bytes]]:
    ...
    page = [
        ...
        f"# {title}",
        "",
        f"> This page is generated by executing `examples/notebooks/{path.name}`.",
        f"> Source: {REPO_BLOB}/examples/notebooks/{path.name}",
        "",
...
def main() -> int:
    ...
    for path in notebooks:
        ...
        page, images = render(path, execute(path))
        slug = PAGES[path.stem]["slug"]

        targets: list[tuple[pathlib.Path, bytes]] = [
            (PAGE_DIR / f"{slug}.md", page.encode())
        ]
        for name, payload in images.items():
            targets.append((IMAGE_DIR / slug / name, payload))
