...
Two things, from one stated price series:
  ...
  1. the model's own market inputs — the daily TBx block prices, which are the
     mean of the most expensive hours a battery of this duration can reach and the
...
nothing. This solves the problem the model is trying to solve:
    ...
    maximize   sum_t price_t * (discharge_t - charge_t)
...
def tbx_blocks(price):
    ...
    most expensive hours it can discharge into and the cheapest it can charge from.
    ...
    def block(hours, expensive):
        col = srt[:, ::-1] if expensive else srt
...
def main():
    ...
    def points(values):
        return "\n".join(
            f"  {(START + dt.timedelta(days=i)).isoformat()}: {v:.4f}"
            for i, v in enumerate(values))

    with open(HERE / "prices.cfdl", "w", newline="\n") as fh:
        fh.write(
            "// The day's achievable prices at this battery's duration, one\n"
            "// point per day. Generated by `reference_gen.py` from the stated\n"
            "// price year; edit that script rather than these numbers.\n"
            "//\n"
            "// `capture_price` is the mean of the most expensive hours the\n"
            "// battery can discharge into, and `cost_price` the mean of the\n"
            "// cheapest it can charge from. How many hours each covers follows\n"
            "// from the battery's power and usable energy.\n\n"
            "curve capture_price {\n" + points(capture) + "\n}\n\n"
            "curve cost_price {\n" + points(cost) + "\n}\n")
