...
The starter pays both loans for the full two years — 15,700 a month of interest on a deal that refinances at month twelve. Add the lifecycle: guard each loan on `entity.status`, and add the event that sets it to `"refinanced"` at `time.t >= 12`.
