...
pub const EXPR_BUILTINS: &[(&str, &str, &str)] = &[
    ...
    ("quantile_at", "quantile_at(name, share)", "The value of a model-declared `quantile` at a cumulative share. The share-indexed analogue of `curve_value`."),
    ("quantile_mean", "quantile_mean(name, from, to)", "The mean value of a `quantile` over a share slice — a partial expectation, taken as the exact integral of the interpolated function."),
    ("quantile_of", "quantile_of(name, value)", "The share at or below which `value` sits. The inverse of `quantile_at`, for slicing against a stated threshold rather than a percentile."),
