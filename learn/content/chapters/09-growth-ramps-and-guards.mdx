...
Read the fraction as "months elapsed over months to stabilize." Variants are one edit each:

- A delayed ramp starts the clock later: `clamp((time.t - 6) / 18.0, 0.0, 1.0)` gives nothing for six months, then the climb.
- A ramp *down* mirrors it: `1.0 - clamp(...)` for a sunset clause or a declining subsidy.
- An S-curve replaces the straight line when the deal justifies the sophistication.

The ramp multiplies a *stabilized* claim. That keeps two assumptions a committee argues about separately — how much at full run-rate, and how long to get there — in two visibly separate places.
...
**A ramp clock in integer time.** `time.t / 18` and `time.t / 18.0` agree here, because arithmetic is decimal throughout. Write the denominator with the decimal point anyway. The reviewer reading `18.0` knows instantly that a fraction was intended, not integer bucketing with `round_down`. Legibility is the habit even where the engine forgives.

**A guard that references what it gates.** `active when` on stream X cannot read X's own series — the condition would gate its own input. The refusal comes from the evaluation-order rule you will meet in the machinery chapter. Streams evaluate in dependency order, and a stream reading its own output is a cycle — the one dependency no order can satisfy. Chains are fine to any depth (a fee may read a fee that reads revenue); only a genuine loop is refused, with its path named.
