...
Stating the number rather than deriving it is a choice with a tradeoff. A stated window is visible and committee-editable, but it can drift from the model that surrounds it. The pack's `cre.exit_forward` derives the same window from the modeled streams instead, at the price of a projection tail. The exercise has you reconcile the two.
