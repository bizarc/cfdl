...
1. **Which streams feed it?** Names answer this — a total over `practice.*` is legible because the taxonomy was designed in the model.
2. **Which periods?** Open the series; find where the cash actually lands. Half of all surprises are timing, not magnitude.
3. **What run settings shaped it?** Rate, as-of date, scenario overrides — all recorded in the output itself. A results document answers "how was this run?" without needing the person who ran it.
4. **What does the model say?** Only now open the source — and you arrive already knowing which stream, which periods, and which claim to read.
...
1. By hand, compute this model's undiscounted total (shown above) and verify it against the run. Then lower the discount rate until NPV approaches the undiscounted total and explain, in one sentence, why they converge.
...
3. Add a fourth stream — an assistant hired from 2026-07 at 5,200 a month — predict the new lifetime total *before* running, then run and confirm. Prediction-before-run is the habit that catches wrong models while they are still cheap.
4. Change the retainer amount to `round_to(30000 / 7, 2) * 7` (a contrived per-diem-style calculation). Look at what one period actually pays, and explain where the pennies went — you have just performed, in miniature, every rounding reconciliation you will ever do.
