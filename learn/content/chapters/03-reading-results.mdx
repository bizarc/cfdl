...
**Slices.** A model that declares slices publishes each one: the selection it stated, the streams it matched, a net series, and total/NPV/IRR over the match. A slice never carries a reconciliation block — it is partial by design, and the document shows it as partial.
