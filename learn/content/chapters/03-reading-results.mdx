...
**Signed convention.** In results, direction becomes sign: inflows positive, outflows negative. Period 1 of this model nets 30,000 − 24,000 − the 150,000 purchase = −144,000; period 2 nets 6,000; period 13 nets 30,000 + 12,000 − 24,000 = 18,000. The signs were declared in the model, which is why the engine can also *check* them — an outflow stream that nets positive over its life is flagged rather than folded quietly into the total.
...
**Metrics.** NPV, IRR, and — once packs are involved — domain metrics like coverage ratios. Each metric carries its inputs in the output (the discount rate used, the as-of date), so a metric is never a bare number you have to reconstruct context for. The prefix says who minted the number: `model.*` is the engine's, `domain.*` the active pack's, and `metric.<name>` one the *model* declared — a figure the deal solved for, evaluated once at the horizon over the finished projection (Part III introduces declaring them).

**The graph, and who owns what.** A top-level `graph` lists every entity — its symbol, family, type, `part of` parent, and (when the model carries one) its stable `id`. Each stream series states its owning `entity` and its `category` beside the values. Together these mean the document stands alone: you can attribute any stream's cash to the thing that owns it without the model open in another window.
...
**Two hashes.** The document states a `model_hash` — identifying the compiled model, taken *without* its views (slices and statements organize; they produce no cash) — and a `ledger_hash` over what came out: the series, the journal, the transitions. The machinery chapter in Part II tells the full story; for now, know that "same model?" and "same result?" are separately answerable questions.
