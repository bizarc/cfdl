...
**`cre.permanent_debt`** → `loan.permanent_debt_service` (financing.debt.service, outflow) — one combined stream, which is what the pack's DSCR metric selects. Interest-only while inside `io_months`, then the level payment struck on `amort_months`; maturity is the contract's term.
