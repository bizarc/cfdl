...
**`cre.revenue_line{.<id>}`** → `cre.revenue.line{.<id>}` (operating.revenue.other, inflow); **`cre.opex_line{.<id>}`** → `cre.opex.line{.<id>}` (operating.expense.opex, outflow). Instanced per line, and both take the same amount spellings:

| Term | Required | Default | Meaning |
|---|---|---|---|
| `amount` | one of the two | 0 | Per-period amount |
| `amount_year` | one of the two | 0 | Annual spelling; the two are additive |
| `escalation` | no | 0 | Stepped on anniversaries; may hold an expression |
| `pct_fixed` | no | 1 | Opex only: the fixed share; the rest scales with `occupancy` |
| `occupancy` | no | 1 | Opex only: occupancy for the variable share |
...
**`cre.percentage_rent[.suffix]`** → `cre.pct_rent.overage[.suffix]` (operating.revenue.percentage_rent, inflow). Overage: `max(0, sales − breakpoint) × pct`.
...
**`cre.permanent_debt`** → `cre.debt.proceeds` (financing.debt.proceeds, inflow), `cre.debt.interest` (financing.debt.interest_paid, outflow) and `cre.debt.principal` (financing.debt.principal, outflow) — the whole instrument, and interest plus principal is the level payment the pack's coverage subtotal reads. Interest-only while inside `interest_only_months`, then the level payment struck on `amortization_months`; maturity is the contract's term, and the loan's balance is an account.

**`cre.mortgage_insurance`** → `cre.mortgage_insurance.premium` (financing.debt.mortgage_insurance, outflow) — `premium_rate` a year on `coverage`, paid on the calendar beside the loan. Not debt service; its own agreement.

| Term | Required | Default | Meaning |
|---|---|---|---|
| `principal` | yes | — | Loan amount |
| `rate` | yes | — | Annual rate |
| `amortization_months` | yes | — | Strikes the payment; normally longer than the term |
| `interest_only_months` | no | 0 | Interest-only period |
...
**`cre.exit_cap`** → `cre.exit.sale` (investing.disposal.reversion, inflow) = `income / cap_rate`. You state the NOI. | Required: `noi_value`, `exit_cap`.

**`cre.exit`** → `cre.exit.proceeds` = `income / cap_rate` **gross**, with `cre.exit.selling_costs` (investing.disposal.selling_costs, outflow) lowering beside it — gross less costs is the net, and the statement shows both. You state a *named forward window* — reconcile it against the modeled final year. | Required: `noi_forward_year`, `exit_cap`. Default: `selling_costs = 0`.

**`cre.exit_forward`** → the same two streams, with forward NOI *derived* from the modeled streams over the twelve months after the sale date. It cannot drift and cannot be committee-edited, and it requires a projection tail so the window has data — with the operating terms extended through the tail, so the forward year exists to be read. | Required: `exit_cap`. Default: `selling_costs = 0`.
