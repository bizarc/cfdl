...
**`cre.revenue_line{.<id>}`** → `cre.revenue.line{.<id>}` (operating.revenue.other, inflow); **`cre.opex_line{.<id>}`** → `cre.opex.line{.<id>}` (operating.expense.opex, outflow). Instanced per line; `amount` or `amount_year`, with `escalation` (which may hold an expression).

| Term | Required | Meaning |
|---|---|---|
| `amount` | yes | Per-period amount |

**`cre.opex_line`** → `cre.opex.line[.suffix]` (operating.expense.opex, outflow).
...
**`cre.exit_forward`** → `cre.exit.proceeds`, with forward NOI *derived* from the modeled streams over the twelve months after the sale date. It cannot drift and cannot be committee-edited, and it requires a projection tail so the window has data. | Required: `exit_cap`. Default: `selling_costs = 0`.
