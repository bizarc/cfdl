...
4. **If it is structure, not opinion** → leave it literal. (Twelve months in a year, a contractual `net 45`, a stated loan principal in a signed document.) Signed structure can itself be a formula, such as a rent escalating at "CPI plus 50 basis points". The term then states that formula: `escalation = curve_value("cpi", time.date) + 0.005`. What was agreed stays in the contract either way; only opinions move to the assumption page.
