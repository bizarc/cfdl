...
1. **Would a reviewer question it?** → an `assume`, on the assumption page. (Growth, margins, pricing, costs.)
2. **Does the question vary per run, per scenario, per user?** → `cfg.*`, in the run configuration. (Stress factors, toggles, the discount rate itself.)
3. **Does the world supply it after the model is written?** → `obs.*`. (Fixings, actuals, appraisals.)
4. **Is it structure, not opinion?** → leave it literal. (Twelve months in a year, a contractual `net 45`, a stated loan principal in a signed document.)
...
2. From a spreadsheet you know well, list five numbers and assign each a home — assume, cfg, obs, or literal — using the four questions. The two you hesitate on are the interesting ones: they are usually claims masquerading as structure.
