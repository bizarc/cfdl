...
- *"At some point, the regime changes — once."* → an **event**. A turning point, latched.
- *"The regime changes and can change back."* → a **lifecycle**. Declared states, guarded edges, no latch — the breach that cures is topology, not a re-firing trigger.
...
7. **A pair of events fighting over one field.** A set-and-unset pair, or a condition and its negation on the same status — a regime that returns, wearing two latches; it is one lifecycle with two edges.
