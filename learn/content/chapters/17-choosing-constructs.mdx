---
title: "Stream, contract, field, event, option, or waterfall?"
...
Part II gave you six ways to say things. Every working session now begins with the same silent decision: *which one is this claim?* Most modeling reviews that go badly go badly here — not on wrong numbers but on wrong *constructs*, economics that are technically computed and structurally illegible. This chapter makes the decision explicit: one sorting question, calibration through pairs, and the smells that tell you a claim has been poured into the wrong shape.
