...
1. Model your own workspace — home office, rented desk, whatever is real to you — on a 24-month grid: one entity, one recurring cost stream, one one-time setup cost. Run it and check the total is what your own arithmetic says.
2. Add a `fitout` phase covering the first two months and move the setup cost's schedule to `on phase_enter("fitout")` — the schedule form that means "once, when this phase begins." Confirm the cash lands in the same period as before. You have just made a date-free claim: slide the phase and the cost follows.
3. Trigger all three refusals above, in any order, and read each message before fixing it. Ten minutes of deliberate breakage now buys you fluent error-reading for the rest of the course.
