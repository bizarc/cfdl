...
The **family** (`asset`, `party`, or `container` for a grouping such as a fund) and the **name** (`tower`, `lender`) combine into the reference you use everywhere else: `asset.tower`, `party.lender`. The **type** after the colon (`Asset.Real`, `Party`) says what kind of thing this is in a vocabulary the tooling understands — real assets, financial assets, intangibles, parties to a transaction. When you later adopt an industry pack, the pack extends this vocabulary with domain types and checks your entities against it — and each pack type states what it specializes (`CRE.Asset.RealProperty` refines `Asset.Real`), so "is a" is a recorded fact rather than a naming convention. Refinement carries substance, not just a label: a type inherits every field its master declares, so what you learn about a master holds for everything downstream of it. Contracts have masters too — the language ships abstract ones like `Contract.Debt` and `Contract.Lease` that exist only to be refined, never instantiated — and contracts check that they attach to an entity of a type that makes sense. A lease on a `Party` is a nonsense the compiler can refuse only because the model said what things *are*, not just what they are called.

Entities can also nest — a building that is `part of` a portfolio, a unit that is `part of` a building — which gives roll-ups something real to roll up along. `part of` is one relation from a small declared vocabulary the ontology carries: a party `owns` an asset or a fund, a contract is `secured_by` the asset behind it, a party `guarantees` a contract or `is_counterparty_to` one. Today these edges are declarative — validated and published in the results graph, driving no cash themselves — but they are how a model records who holds what, and what stands behind which obligation, as facts rather than as comments. The capstone uses the nesting; for now just know the structure is there.
...
Start a habit now for reading any model. Find the grid, count the cast, then classify every stream as one of the three kinds of claim a stream can be:

- **Scheduled** — dates known, amount known. The fitout.
- **Recurring** — dates patterned, amount ruled. The rent.
- **Contingent** — existence itself depends on something, such as an option exercised or a covenant tripped. You have not met this kind yet.

Part II closes the gap between the second and third kind.
