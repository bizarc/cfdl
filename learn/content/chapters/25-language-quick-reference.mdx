...
Reads, from any expression: `asset.x.field` (this period, at close), `prev.asset.x.field` (prior close), `entity.state.field` (the attached entity's own field). Rules see only the completed prior column — same-period cycles are unwritable.
...
Placement: default = period end (arrears); `due` = start (advance; stride schedules only — one payment in advance is `every month due from X to X`); `mid` = midpoint (mid-period convention). Conventions: `none`, `following`, `preceding`, `modified_following`, `modified_preceding`. Calendars: `"weekend"`, `"us"`, `"uk"`, `"target"`. `net` counts days, or steps calendar months. Occurrences pushed off the grid are compile errors.
