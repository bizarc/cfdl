...
A waterfall has a schedule (it distributes on dates, like any claim), a **pot** — the `from` expression, and `available` is the trust's own netted cash for the period, handed over by the engine the way `remaining` is — and ordered **steps**. Each step names itself, names its payee (an entity reference — this is where chapter 2's parties earn their place in the cast), and states what the step is *owed* as an ordinary expression.
