...
1. **Check that the base case sits near the median.** If the deterministic NPV is far from p50, your central values and your distributions disagree about the deal — usually a skewed input (that LogNormal) pulling the mean away from the mode. Understand why before quoting either number.
...
3. **Count the fraction of trials that go negative.** "Eleven percent of paths lose money" is the sentence that changes meetings — concrete, checkable, and impossible to produce from a base case and a downside case alone.
