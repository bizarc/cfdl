...
The pack's lease lowers to a rent stream over the term with chapter 9's ramp built in — `lease_up_months` *is* the `clamp` idiom, as a term. Twelve months from a July 2027 start puts stabilization at mid-2028, which is why the skeleton's phases drew the lease-up phase where they did. Phases and terms telling the same story is not a coincidence to admire. It is a consistency to maintain.
