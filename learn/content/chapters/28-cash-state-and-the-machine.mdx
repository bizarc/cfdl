...
bookkeeping that belongs to it — reset a counter, record a shortfall, strike a
new rate — belongs on the arrival rather than in a separate event that has to
restate the condition.

Here is a loan that misses payments, defaults, resumes paying, and is restored
to performing only after a **probation period**. That last requirement is why
the counter has to exist: "three consecutive good months" is not a condition
you can read off the calendar, and a plain recurrence counts up forever
without ever going back to zero.
...
entity asset loan {
  ...
  months_in_state init 0.0 next prev + 1.0
}
...
lifecycle servicing {
  ...
  // Every arrival restarts the clock. This is what makes "three months in
  // this condition" sayable at all.
  on enter current    { set months_in_state = 0 }
  on enter delinquent { set months_in_state = 0 }
...
default in the first place. Without arrival actions you cannot express it: the
counter would still be climbing from whenever the model started.

Two grains, and both are real:

- **`on enter <state>`** carries what is true of the STATE however it was
  reached. Resetting the clock holds for every edge that arrives, including one
  you add later.
- **an action on an edge** carries what is true of the PATH taken. A renewal
  ...
  because of how you arrived — and an entry action cannot say that, since it
  does not know which edge fired.
