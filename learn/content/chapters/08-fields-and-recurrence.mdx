...
A **fact field** (`seniority = 1`) takes a literal and nothing else. It is a statement about the thing, checked against the pack's ontology when one is in play, so a misspelled field name fails at compile time.

A **rule field** carries the recurrence. **`init`** is the value at the first period — an expression, since a base case often needs `inputs.*`. **`next`** is evaluated each subsequent period, with **`prev`** bound to this field's own value one period ago. A computed "fact" (`occupancy = base * 0.95`) is deliberately illegal — a value that moves is a rule, and the syntax makes you say so.
...
The cost is one modeling discipline: genuine same-period simultaneity must be *staged*. Some definitions truly need this period's answer on both sides: price that depends on volume that depends on price. Restate such a definition as a rule off last period, or split it across two fields. Either way, the model *declares its convention*. That is the trade throughout this language: a construct that cannot express confusion, in exchange for saying what you mean.
...
And a field is not the answer to everything sequenced. Sometimes the accumulating thing is *someone's entitlement*: arrears owed, or a preferred return accruing until paid. The language has purpose-built constructs for that — `waterfall`'s `owed` and `paid`, chapter 11 — which carry the entitlement *and* the payment against it together. Hand-building that from raw fields means re-proving what the waterfall already proves. The smell to notice: a field paired with a stream that tries to "pay it down."
...
**`prev` at the very first period.** There is no close before the first close. A stream that reads `prev.asset.x.y` in period 0 is refused rather than handed a silent zero. Schedule such streams from the second period on, as the average-balance example does.
