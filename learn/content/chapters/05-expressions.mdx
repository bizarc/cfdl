...
- **`state.*`** and **`prev`** — the model's accumulated state (chapter 8).
- **`entity.*`** — fields of the entity the stream is attached to.
...
**Rounding.** `round`, `round_up`, `round_down`, `round_to(x, places)`. Two distinct uses: `round_down(time.t / 12, 0)` for step functions, and `round_to(…, 2)` for matching a source that rounds each period before summing — the reconciliation tool from chapter 3.
