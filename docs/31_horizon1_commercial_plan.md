...
computed in the language. Both dependencies have now shipped: the account
(`docs/28` §5.1) and the declared metric (§7.25).
...
| 3 | **Benchmark case** — a published GP/LP waterfall worked example (European and American carry, preferred, catch-up, clawback test) reconciled tier-by-tier and party-by-party; register with the site | The pack question — whether tiers warrant contracts or stay core waterfall spellings — is answered by this case, not before it. `penzance_highlands` already hand-assembles Baupost's return; converting it is the second case and the §7.3 coverage proof — **that conversion is done** (2026-08-29): the JV split allocates from `account deal_cash`, both partners hold party-owned accounts, and `irr`/`moic` per party are published metrics rather than hand arithmetic. The GP/LP worked example is what remains of this phase — and it is the half that carries the site registration, since Highlands is deliberately unpublished (`sync-content.mjs` `benchmarkUnpublished`: a named private partnership with assumed JV economics). |
