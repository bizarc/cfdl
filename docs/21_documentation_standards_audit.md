...
- **No glossary exists** anywhere in `site/`, `learn/`, or `docs/`. Both
...
- **No `description` frontmatter on any of the 111 site doc pages.** Every page
...
| 9 | Convert `learn` procedural sections to imperative steps | Days | §4.6; bounded to the identified sections, not whole chapters |
| 10 | WCAG 2.2 AA assessment and remediation | Unknown until assessed | Legal baseline in the EU; cannot be scoped before it runs |
| 11 | Extend `check-site-voice.py` with CFDL-CE rules | Days | Makes 1–9 stay fixed |
