...
| `event <name> when <expr> { actions }` | ✅ | latch semantics: fires once, at the first period its condition holds, in declaration order. The guard reads state as the period OPENED and series STRICTLY BACKWARD (`docs/28` §4) — settled cash at or before the previous period; same-period and forward reads are refused (`E1134`). A regime that returns is not a re-firing event but a lifecycle edge |
...
| — `activate/deactivate contract` | 🟡 | parsed + lowered; engine warns-and-skips and journals the action as `ignored` (no contract runtime yet). UNREACHABLE FROM A MODEL: a contract carries only its type, so the reference cannot resolve (`E1303`) — backlog 7.63 sequences before the runtime, because the runtime needs an instance to name |
