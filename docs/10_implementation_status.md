...
| `account <name> { owner <ref>  from <expr> }` | ✅ | a declared cash location under the balance law; `from <account>` as a waterfall's pot, `pay <step> to account <name>` credits it, a party-owned account receives that party's allocations, `prev.<account>` is settled history in logic and steps. Balance publishes as a non-cash `account.<name>` series; every movement journaled (`inflow`, `allocate_in`, `allocate_out`). `E1347` refuses an undeclared account payee |
