...
| — `activate/deactivate contract` | ⬛ | **retired** (backlog 7.73). A contract is a collection of streams and one switch cannot say what forbearance says; the retired spelling is `E0006_RETIRED_SYNTAX`, and the replacements are a lowered stream by name (7.50) or `active in state` |
