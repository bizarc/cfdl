...
| master contract types (`refines`, `abstract`, `type <Master>` in a slice) | 🟡 | seventeen masters in the language base declare fields, roles, lines and side (`docs/40` §4); refinements inherit them, specialize roles (`landlord refines lessor`) and are checked at pack load (strengthen-only fields, every master role covered, line coverage, template coverage, and every term a rule reads, a validation bounds or a template renders declared as a field). Every shipped pack type declares its terms as fields under the masters' names. The compiler checks a model's terms against the effective roster (`E1371`, `E1372`), checks the type named on a contract or an option (`E1373`, `E1374`), and carries the resolved type, master, instance and parties in the IR. Results publish the contracts in `graph.contracts` with type, master, parties and lowered streams, and each lowered series names its contract (`results_version` 0.13); MCP `lookup` reads each type against its master chain. Not yet: `type <Master>` on metrics and statement rows (stage 5), `balance_field` (stage 6), the option grammar aligned with `Contract.Option` (stage 7) — `docs/40` §10 |
