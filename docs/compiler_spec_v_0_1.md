...
- Build symbol tables for entities, contracts, streams, phases, assumptions, options, events, metrics.
...
- `required_observables`: all unique IDs referenced in `obs('...')` calls inside CEL strings
- `required_refs`: all unique IDs referenced in `ref('...')` calls inside CEL strings

**v0.1 requirement:**
- Implement a **lexical extractor** (regex/finite scan) that finds `obs('X')` / `obs("X")` and `ref('Y')` / `ref("Y")`.
