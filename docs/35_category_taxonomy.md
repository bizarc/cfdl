...
larger half of what was `docs/13` §7.55, and it shipped after this landed
(§6, question 3).
...
## 6. Questions, decided 5 September 2026

1. **May a model open a node the pack left closed?** Yes. The three roots are
   the only gate; a pack's list is the domain's conventional spelling, and a
   well-rooted category it does not list is valid, with a near-miss warning
   (`W5023`). A pack cannot enumerate every line a deal has.
2. **Should the uncategorized-stream warning be an error behind a strict
   flag?** No. The language has no modes and gains none. A category is part
   of a stream's definition: under a pack its absence is already an error
   (`E5029`), and only a pack-free model may omit it, because nothing folds
   there. A benchmark case is held to the stricter rule by the harness, not
   by a switch.
3. **Does the subtree row defer model-declared statements?** Moot: the
   statement shipped in full on 1 September 2026 — generated by entity or
   category tree, or authored row by row, with ratios over slices and the
   display sign (`docs/01` §16; `E1366`–`E1370`). A model needs no pack to
   present itself.
...
Presentation is unchanged here; the model-declared statement shipped later (§6) and
remains open.
