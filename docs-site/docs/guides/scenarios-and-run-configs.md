...
slug: /guides/scenarios-and-run-configs
---
...
the [Python SDK](/python-sdk), where `results.scenarios()` lands each
...
[Stochastic Modeling](/stochastic-modeling) and the
[run-config reference](/reference/run-config).

## Reference links

- [Run-Config Reference](/reference/run-config)
- [CLI Reference — cfdl run](/reference/cli)
